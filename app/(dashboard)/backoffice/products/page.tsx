...
export default function ProductsPage() {
  ...
  const { products, categories, rawMaterials, variants, deleteProduct, deleteCategory, addCategory, addProduct, updateProduct, updateCategory, addVariant, updateVariant, deleteVariant } = useInventoryStore();
  const [activeTab, setActiveTab] = useState<'PRODUCTS' | 'CATEGORIES' | 'VARIANTS'>('PRODUCTS');
  ...
  const [variantForm, setVariantForm] = useState<any>({ name: '', isRequired: false, type: 'SINGLE_CHOICE', options: [] });
  const [deleteVariantConfirm, setDeleteVariantConfirm] = useState<string | null>(null);
  
  const filteredProducts = products.filter(p => p.name.toLowerCase().includes(search.toLowerCase()));
  const filteredCategories = categories.filter(c => c.name.toLowerCase().includes(search.toLowerCase()));
  const filteredVariants = variants?.filter(v => v.name.toLowerCase().includes(search.toLowerCase())) || [];

  const handleOpenAddProduct = () => {
    setEditingProductId(null);
    setProductForm({ name: '', categoryId: categories[0]?.id || '', basePrice: 0, type: 'SINGLE', variantIds: [], comboItems: [] });
    setMoneyText('');
  ...
  const handleOpenEditProduct = (p: Product) => {
    ...
    setProductForm({ name: p.name, categoryId: p.categoryId, basePrice: p.basePrice, type: p.type, variantIds: p.variantIds || [], comboItems: p.comboItems || [] });
  ...
  const handleOpenAddVariant = () => {
    ...
    setVariantForm({ name: '', isRequired: false, type: 'SINGLE_CHOICE', options: [] });
         ...
         <div className="flex flex-col md:flex-row justify-between gap-4 mb-4">
            <div className="relative w-full md:w-96">
             ...
             {activeTab === 'CATEGORIES' && (
               <Button onClick={handleOpenAddCategory} className="gap-2 bg-[#00E5FF] text-black hover:bg-cyan-300 border-2 border-black shadow-[2px_2px_0_0_#000]">
                 <Plus className="w-5 h-5"/> KATEGORI BARU
               </Button>
             )}
             {activeTab === 'VARIANTS' && (
               <Button onClick={handleOpenAddVariant} className="gap-2 bg-[#FF90E8] text-black hover:bg-pink-300 border-2 border-black shadow-[2px_2px_0_0_#000]">
         ...
         <div className="overflow-x-auto border-4 border-black rounded-xl">
            <Table>
               <TableHeader className="bg-gray-100 border-b-4 border-black font-space-grotesk font-black uppercase">
                 ...
                 {activeTab === 'VARIANTS' && (
                   <TableRow>
                     <TableHead className="text-black border-r-2 border-black">Nama Varian</TableHead>
                     <TableHead className="text-black border-r-2 border-black">Tipe & Sifat</TableHead>
                     <TableHead className="text-black border-r-2 border-black">Jumlah Opsi</TableHead>
                     <TableHead className="text-right text-black w-[150px]">Aksi</TableHead>
                   </TableRow>
                 )}
               </TableHeader>
               <TableBody className="font-inter font-bold">
                 ...
                 {activeTab === 'VARIANTS' && filteredVariants.map(v => (
                    <TableRow key={v.id} className="border-b-2 border-gray-200 hover:bg-gray-50">
                      <TableCell className="border-r-2 border-gray-200 uppercase font-black">{v.name}</TableCell>
                      <TableCell className="border-r-2 border-gray-200 text-sm">
                        <span className={`px-2 py-1 mr-2 border-2 border-black rounded-md ${v.type === 'SINGLE_CHOICE' ? 'bg-cyan-100' : 'bg-purple-100'}`}>
                          {v.type === 'SINGLE_CHOICE' ? 'Pilih Satu' : 'Pilih Banyak'}
                        </span>
                        <span className={`px-2 py-1 border-2 border-black rounded-md ${v.isRequired ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                          <button onClick={() => handleOpenEditVariant(v)} className="p-2 border-2 border-black rounded hover:bg-gray-100 transition-colors">
                            <Edit2 className="w-4 h-4"/>
                          </button>
                          <button onClick={() => setDeleteVariantConfirm(v.id)} className="p-2 border-2 border-black rounded bg-red-100 text-red-600 hover:bg-red-200 transition-colors">
                            <Trash2 className="w-4 h-4"/>
                          </button>
                        </div>
                      </TableCell>
                 ...
                 {((activeTab === 'PRODUCTS' && filteredProducts.length === 0) || (activeTab === 'CATEGORIES' && filteredCategories.length === 0) || (activeTab === 'VARIANTS' && filteredVariants.length === 0)) && (
                    <TableRow>
        ...
        <DialogContent className="border-8 border-black rounded-[2rem] bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] max-w-xl max-h-[90vh] overflow-y-auto">
           ...
           <div className="flex flex-col gap-4 py-4">
              ...
              <div className="flex flex-col gap-2 border-4 border-black p-4 rounded-xl mt-2 bg-gray-50">
                 <Label className="text-lg font-space-grotesk font-black uppercase">Pilihan Varian</Label>
                 <p className="text-xs font-inter font-bold text-gray-500 mb-2">Pilih varian yang berlaku untuk menu ini.</p>
                 <div className="flex flex-wrap gap-2">
                   {variants && variants.length > 0 ? variants.map(v => {
                     ...
                     return (
                       <button
                         ...
                         onClick={() => {
                           ...
                           if (isSelected) {
                             setProductForm({...productForm, variantIds: curr.filter(id => id !== v.id)});
                           } else {
                             setProductForm({...productForm, variantIds: [...curr, v.id]});
                           }
                         }}
                     ...
                     <div className="text-sm font-bold text-gray-400 italic mb-2">Belum ada data varian. Tambahkan di tab Varian Menu.</div>
                   )}
                 </div>
              </div>
        ...
        <DialogContent className="border-8 border-black rounded-[2rem] bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] max-w-lg max-h-[90vh] overflow-y-auto">
           ...
           <div className="flex flex-col gap-4 py-4">
              <div className="flex flex-col gap-2">
                 ...
                 <Input value={variantForm.name} onChange={e => setVariantForm({...variantForm, name: e.target.value})} placeholder="Contoh: Ukuran / Topping / Rasa" />
              </div>
              <div className="flex flex-col md:flex-row gap-4">
                 <div className="flex flex-col gap-2 flex-1">
              ...
              <div className="flex flex-col gap-2 border-4 border-black p-4 rounded-xl mt-2 bg-gray-50">
                  <div className="flex justify-between items-center mb-2">
                    <div>
                      <Label className="text-lg font-space-grotesk font-black uppercase">Opsi Pilihan</Label>
                      <p className="text-xs font-inter font-bold text-gray-500">Harga opsional. Biarkan 0 jika gratis.</p>
                    </div>
                    <Button 
                      size="sm" 
                      onClick={() => setVariantForm({...variantForm, options: [...(variantForm.options || []), { name: '', priceAdjustment: 0 }]})}
                  ...
                  <div className="flex flex-col gap-2">
                     {variantForm.options?.map((opt: any, idx: number) => (
                        <div key={idx} className="flex gap-2 items-center bg-white p-2 border-2 border-black rounded-lg">
                           ...
                           <Input 
                             ...
                             onChange={(e) => {
                               const newOps = [...variantForm.options];
                               newOps[idx].priceAdjustment = parseInt(e.target.value) || 0;
                               setVariantForm({...variantForm, options: newOps});
                             }}
                     ...
                     {(!variantForm.options || variantForm.options.length === 0) && (
                        <div className="text-sm text-center text-gray-500 font-bold py-4">Belum ada opsi. Silahkan tambah opsi.</div>
                     )}
                  </div>
              </div>
           </div>
           <DialogFooter>
             <Button variant="outline" onClick={() => setIsVariantModalOpen(false)}>BATAL</Button>
