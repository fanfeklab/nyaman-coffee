...
const generateId = (prefix: string) =>
  `${prefix}_${Date.now().toString(36)}_${Math.random().toString(36).substring(7)}`;
...
export default function ManualSalesPage() {
  ...
  const handleSave = async () => {
    ...
    if (items.length === 0)
      return toast.error("Belum ada item yang ditambahkan");
    if (items.some((i) => !i.productId || i.qty <= 0))
      return toast.error("Pastikan semua item valid");
  ...
  return (
    <div className="p-6 md:p-10 max-w-5xl flex flex-col gap-8 h-full overflow-y-auto hide-scrollbar">
      <div>
        <h1 className="font-space-grotesk font-black text-4xl uppercase tracking-widest text-black mb-2">
          INPUT PENJUALAN MANUAL
        </h1>
        <p className="font-inter font-bold text-gray-500">
          Fitur khusus Super Admin / SPV untuk memasukkan histori transaksi masa
          lalu.
        </p>
      </div>
      ...
      <div className="bg-white border-4 border-black p-6 rounded-2xl shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex flex-col gap-6 animate-in fade-in zoom-in-95 duration-200">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            <FormSelect
              ...
              options={[
                ...users.map(u => ({ value: u.username, label: `${u.fullName} (${u.username})` })),
                { value: 'kasir_lainnya', label: 'Lainnya...' }
              ]}
            />
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <FormSelect
            ...
            className="h-12 w-full md:w-1/2 border-2 border-black"
        ...
        <div className="border-t-4 border-black pt-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="font-space-grotesk font-black text-2xl uppercase">
              Daftar Item Dibeli
            </h2>
            <Button
              onClick={handleAddItem}
              className="bg-black text-white hover:bg-gray-800 font-space-grotesk font-black uppercase border-2 border-black"
            >
          ...
          {items.length === 0 ? (
            <div className="p-8 border-4 border-dashed border-gray-300 rounded-xl text-center text-gray-500 font-bold">
              Belum ada item ditambahkan. Klik tombol di atas untuk menambah
              barang.
            </div>
          ) : (
            <div className="flex flex-col gap-4">
              {items.map((item, idx) => (
                <div
                  key={idx}
                  className="flex flex-col sm:flex-row gap-2 items-center bg-gray-50 p-2 border-2 border-black rounded-lg"
                >
                  <div className="flex-1 w-full">
                    ...
                    <FormSelect
                      ...
                      className="h-10 border-2 border-black shrink-0"
                      placeholder="Pilih Menu"
                      options={products.map(p => ({
                        value: p.id,
                        label: `${p.name} - Rp ${p.basePrice.toLocaleString()}`
                      }))}
                    />
                  </div>
        ...
        <div className="bg-[#FFD100] border-4 border-black p-4 rounded-xl flex flex-col md:flex-row items-start md:items-center justify-between shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] mt-4 gap-4">
          <div className="w-full md:w-auto">
            <span className="font-space-grotesk font-black uppercase text-gray-800 text-sm block">
          ...
          <Button
            onClick={handleSave}
            className="w-full md:w-auto h-14 text-sm md:text-lg bg-[#00E5FF] text-black hover:bg-cyan-400 font-space-grotesk font-black uppercase border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-y-1 hover:shadow-none transition-all"
          >
